...
try {
    ...
    Copy-Item -LiteralPath (Get-Command node).Source -Destination $outputExe -Force
    node (Join-Path $PSScriptRoot 'strip-signature.mjs') $outputExe
    node $postjectCli $outputExe NODE_SEA_BLOB $blobPath --sentinel-fuse NODE_SEA_FUSE_fce680ab2cc467b6e072b8b5df1996b2
